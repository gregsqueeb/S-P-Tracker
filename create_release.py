...
PUTTY_BASE_PATH = r"C:\Program Files (x86)\putty"
VM_HOST = "192.168.1.77"
...
if len(sys.argv) != 2:
    print ("Usage: create_release [--test_release_process] [--ptracker_only] <version_number>")
    sys.exit(1)

if not test_release_process:
    svn_status = subprocess.check_output(["svn", "status", "-q"], universal_newlines=True)
    if not svn_status.strip() == "":
        print ("svn is dirty. Check in your changes first.")
        print (svn_status)
        sys.exit(1)

svn_url = subprocess.check_output(["svn", "info", "--show-item", "url"], universal_newlines=True)
...
if not test_release_process:
    svn_commit = subprocess.check_output(["svn", "commit", "-m", "prepare release %s" % version])
...
if not ptracker_only:
    ...
    if not test_release_process:
        rbuild_out = subprocess.check_output([PUTTY_BASE_PATH + r"\PLINK.EXE", "-pw", "neys", "neys@" + VM_HOST, "cd ptracker && sh create_release.sh " + svn_url], universal_newlines=True)
        rcopy_out = subprocess.check_output([PUTTY_BASE_PATH + r"\PSCP.EXE", "-pw", "neys", "neys@" + VM_HOST + r":./ptracker/stracker/stracker_linux_x86.tgz", r".\stracker\stracker_linux_x86.tgz"], universal_newlines=True)
